...
public class InventoryBuilder {
    ...
    public Inventory setupInventory() {
        ...
        items.put(new Pair<Integer, Integer>(3, 3), new ItemManager().getItem(Material.COMPASS, 1, "§eBasics"));
        items.put(new Pair<Integer, Integer>(3, 5), new ItemManager().getItem(Material.MAGMA_CREAM, 1, "§eSpawns"));
        items.put(new Pair<Integer, Integer>(3, 7), new ItemManager().getItem(Material.BOOK, 1, "§eGame Profiles"));
    ...
    public Inventory GameProfileInventory() {
        HashMap<Pair<Integer, Integer>, ItemStack> items = new HashMap<Pair<Integer, Integer>, ItemStack>();
        items.put(new Pair<Integer, Integer>(3, 3), new ItemManager().getItem(Material.COMPASS, 1, "§eBasics"));
        items.put(new Pair<Integer, Integer>(1, 1), new ItemManager().getHead(
        ...
        for (int row = 2; row < 4; row++) {
            for (int column = 2; column < 9; column++) {
                items.put(new Pair<Integer, Integer>(row, column),
